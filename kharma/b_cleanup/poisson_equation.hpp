  ...
  public:
        ...
        std::shared_ptr<parthenon::MeshData<Real>>& md_out)
    {
        ...
        if (set_flux_boundary) {
            flux_res =
                tl.AddTask(flux_res, SetFluxBoundaries, md_mat, md_in, include_flux_dx);
        }
        if (do_flux_cor &&
    ...
    KOKKOS_INLINE_FUNCTION auto GetEffectiveInverseDx2(
        const coords_t& coords, const int k, const int j, const int i) const
    {
        ...
        std::shared_ptr<parthenon::MeshData<Real>>& md_diag)
    {
        ...
        auto pkg = md_mat->GetMeshPointer()->packages.Get("B_Cleanup");
        ...
        using TE = parthenon::TopologicalElement;
        parthenon::par_for(
            "StoreDiagonal", 0, pack_mat.GetNBlocks() - 1, kb.s, kb.e, jb.s, jb.e, ib.s,
            ib.e,
    ...
    static parthenon::TaskStatus SetFluxBoundaries(
        std::shared_ptr<parthenon::MeshData<Real>>& md_mat,
        ...
        KOKKOS_LAMBDA(parthenon::team_mbr_t member, const int b)
            {
                ...
                for (int face = 0; face < ndim * 2; ++face) {
                    ...
                    // Correct for size of neighboring zone at fine-coarse boundary when
                    // using constant prolongation
                    if (do_flux_dx && pack.GetLevel(b, x3off[face], x2off[face],
                                          x1off[face]) == level - 1) {
                        parthenon::par_for_inner(DEFAULT_INNER_LOOP_PATTERN, member, 0,
                            idxer.size() - 1,
                            [&](const int idx)
        ...
        std::shared_ptr<parthenon::MeshData<Real>>& md_out)
    {
        ...
        parthenon::par_for("FluxMultiplyMatrix", 0, pack.GetNBlocks() - 1, kb.s, kb.e,
            jb.s, jb.e, ib.s, ib.e, KOKKOS_LAMBDA(const int b, const int k, const int j, const int i)
            {
                const auto& coords = pack.GetCoordinates(b);
