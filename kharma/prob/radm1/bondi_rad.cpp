...
{
    // McKinney et al. 2014, Table 4: mass accretion rate, outer temperature, and the
    // initial radiation-to-gas pressure ratio (used to derive Gamma, eq. 95).
    const Real mdot = pin->GetOrAddReal("bondi_rad", "mdot", 1.0);
    const Real T_out = pin->GetOrAddReal("bondi_rad", "T_out", 1.e6);
