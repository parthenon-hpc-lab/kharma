    ...
    ParameterInput* pin, std::shared_ptr<Packages_t>& packages)
{
    ...
    if (opacity_type == "none") {
        ...
        if (scale_free) {
            ...
            singularity::neutrinos::Opacity opacity_host = NonCGSUnits<Gray>(
...

    {
        ...
        if (scale_free) {
            ...
            const Real lNuMin =
                std::log10(pin->GetOrAddReal("mean_opacity", "numin", 0.1));
            const Real lNuMax =
                std::log10(pin->GetOrAddReal("mean_opacity", "numax", 10.));
            ...
            auto mean_opac_host = MeanOpacityBase(opacity_host, lRhoMin, lRhoMax, NRho,
                lTMin, lTMax, NT, YeMin, YeMax, NYe, lNuMin, lNuMax, NNu);
            ...
            const int NRho = pin->GetOrAddInteger("mean_opacity", "nrho", 10);
            const Real lTMin =
                pin->GetOrAddReal("mean_opacity", "ltmin", std::log10(1.e5));
            const Real lTMax =
                pin->GetOrAddReal("mean_opacity", "ltmax", std::log10(1.e12));
            const int NT = pin->GetOrAddInteger("mean_opacity", "nt", 10);
            const Real lNuMin =
                std::log10(pin->GetOrAddReal("mean_opacity", "numin", 1.e10));
            const Real lNuMax =
                std::log10(pin->GetOrAddReal("mean_opacity", "numax", 1.e24));
            const int NNu = pin->GetOrAddInteger("mean_opacity", "nnu", 100);
...

    {
        ...
        if (scale_free) {
            ...
            const Real lTMax =
                pin->GetOrAddReal("mean_opacity", "ltmax", std::log10(10.));
            const int NT = pin->GetOrAddInteger("mean_opacity", "nt", 10);
            MeanSOpacity mean_opac_host(MeanSOpacityBase(opacity_host, lRhoMin, lRhoMax,
                NRho, lTMin, lTMax, NT, YeMin, YeMax, NYe));
            ...
            const Real lNuMin =
                pin->GetOrAddReal("mean_opacity", "lnumin", std::log10(1.e14));
            const Real lNuMax =
                pin->GetOrAddReal("mean_opacity", "lnumax", std::log10(1.e22));
            ...
            MeanSOpacityBase cgs_mean_opacity(opacity_host, lRhoMin, lRhoMax, NRho, lTMin,
                lTMax, NT, YeMin, YeMax, NYe, lNuMin, lNuMax, NNu);
